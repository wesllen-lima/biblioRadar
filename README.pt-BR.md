...
- Disponibilidade de PDF verificada com requisições HEAD em lote + proxy CORS como fallback
...
npm run lint     # ESLint
npm test         # Suite de testes Vitest (19 testes)
...
**Sem autenticação.** O compartilhamento P2P codifica a biblioteca em um parâmetro de URL comprimido com gzip. Nada passa pelos servidores do BiblioRadar.
...
**Módulos de provedor são isolados.** Cada fonte exporta uma única função `search(query, options): Promise<BookResult[]>`. Adicionar uma nova fonte nativa significa criar um arquivo e registrá-lo no handler de rota.

**Extração de utilitários em vez de duplicação.** Lógica compartilhada fica em módulos focados (`coverUtils.ts`, `graph.ts`, `searchFilters.ts`) em vez de ser duplicada entre componentes. Nenhum arquivo ultrapassa 450 linhas.
...
2. **TypeScript strict mode** — sem `any`. Use a union `SourceId` (`gutenberg | internet_archive | open_library | arxiv | opds | scrape | user`) para identificadores de fonte.
...
5. **Testes** — adicione ou atualize testes Vitest para mudanças de lógica em `lib/`. Execute `npm test` para verificar que todos os 19 testes passam.
